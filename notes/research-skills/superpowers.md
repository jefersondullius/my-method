...
## Addendum (live, 2026-08-13) — manual-copy question, focused on the `brainstorming` skill

**Q1 reconfirmed:** still 14 skills in the plugin, `brainstorming` one of them [https://api.github.com/repos/obra/superpowers/contents/skills — accessed 2026-08-13]. Still no mechanism to disable one skill inside an installed plugin — official docs (`code.claude.com/docs/en/discover-plugins`, accessed 2026-08-13) only describe whole-plugin granularity (`/plugin disable plugin-name@marketplace-name`). A maintainer request to make `brainstorming` (or the plugin generally) manual-invocation-only via `disable-model-invocation: true` was explicitly considered and **rejected**, issue closed "not planned" [https://github.com/obra/superpowers/issues/1456 — accessed 2026-08-13] — this SessionStart-hook behavior is a deliberate author choice, not an oversight.

**Q2 — license and what a copy needs:** MIT License, `LICENSE` at repo root, "Copyright (c) 2025 Jesse Vincent" [https://raw.githubusercontent.com/obra/superpowers/main/LICENSE — accessed 2026-08-13] — permits manual copying. New finding: `skills/brainstorming/SKILL.md` itself references a sibling file in the **same folder**, `skills/brainstorming/visual-companion.md` — copying only the bare `SKILL.md` file breaks that one feature; copying the whole `brainstorming/` folder (both files) preserves 100% of the function standalone [https://raw.githubusercontent.com/obra/superpowers/main/skills/brainstorming/SKILL.md — accessed 2026-08-13]. References to other skills in the pack (e.g. handing off to `writing-plans`) are confirmed to be textual "next step" suggestions only, not technical dependencies.

**Q3 — the trade-off that matters most for this one:** the plugin's `SessionStart` hook (matcher `startup|clear|compact`) injects the **entire** `using-superpowers/SKILL.md` (~5.4KB, ~1,300 tokens) into every session unconditionally — a much larger always-on cost than an ordinary skill description line [https://raw.githubusercontent.com/obra/superpowers/main/hooks/hooks.json — accessed 2026-08-13]. Confirmed technically: hooks live in the plugin root's `hooks/` folder and are only registered when the whole plugin is installed via `/plugin` — a manual copy of just the `brainstorming/` folder into `.claude/skills/` does **not** bring `hooks.json` or the hook script along [https://code.claude.com/docs/en/plugins — accessed 2026-08-13]. So manual copy of `brainstorming/` avoids that ~1,300-token per-session tax entirely, at the cost of the other 13 skills, official support, and auto-updates.

**Status 2026-08-13:** no decision recorded yet for this one — flagged back to the user rather than assumed.
